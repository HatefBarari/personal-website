...
const observeSection = new IntersectionObserver(observerHandler, {
  threshold: 0.5,
});
function observerHandler(allSections) {
  allSections.map((section) => {
    let sectionClassName = section.target.className;
    let sectionMenuItem = $.querySelector(
...
//!!! Default !!!//
const hiddenElementXLeft = document.querySelectorAll(".hidden-x-left");
hiddenElementXLeft.forEach((el) => observer.observe(el));

const hiddenElementXRight = document.querySelectorAll(".hidden-x-right");
hiddenElementXRight.forEach((el) => observer.observe(el));

const hiddenElementYUp = document.querySelectorAll(".hidden-y-up");
hiddenElementYUp.forEach((el) => observer.observe(el));

const hiddenElementYDown = document.querySelectorAll(".hidden-y-down");
hiddenElementYDown.forEach((el) => observer.observe(el));

//!!! Mobile !!!//

const hiddenElementXmbLeft = document.querySelectorAll(".hidden-x-mb-left");
hiddenElementXmbLeft.forEach((el) => observer.observe(el));

const hiddenElementXmbRight = document.querySelectorAll(".hidden-x-mb-right");
hiddenElementXmbRight.forEach((el) => observer.observe(el));

const hiddenElementYmbUp = document.querySelectorAll(".hidden-y-mb-up");
hiddenElementYmbUp.forEach((el) => observer.observe(el));

const hiddenElementYmbDown = document.querySelectorAll(".hidden-y-mb-down");
hiddenElementYmbDown.forEach((el) => observer.observe(el));

//!!! Desktop !!!//

const hiddenElementXdsLeft = document.querySelectorAll(".hidden-x-ds-left");
hiddenElementXdsLeft.forEach((el) => observer.observe(el));

const hiddenElementXdsRight = document.querySelectorAll(".hidden-x-ds-right");
hiddenElementXdsRight.forEach((el) => observer.observe(el));

const hiddenElementYdsUp = document.querySelectorAll(".hidden-y-ds-up");
hiddenElementYdsUp.forEach((el) => observer.observe(el));

const hiddenElementYdsDown = document.querySelectorAll(".hidden-y-ds-down");
hiddenElementYdsDown.forEach((el) => observer.observe(el));
